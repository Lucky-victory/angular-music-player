...
export class PlayerImageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChildren('playerImage', { read: ElementRef })
  playerImage!: QueryList<ElementRef>;
  ...
  onSlideEnd() {
    ...
    if (
      movedBy < -150 &&
      this.currentIndex < this.playerImagesArray.length - 1
    ) {
      this.currentIndex += 1;
  ...
  setPositionByIndex() {
    const playerImagesContainer = this.playerImagesContainer
    ...
    this.currentTranslate =
      this.currentIndex * -this.playerImagesContainer.nativeElement.clientWidth;
  ...
  setSliderPosition() {
    (
      this.playerImageWrapper.nativeElement as HTMLDivElement
    ).style.transform = `translateX(${this.currentTranslate}px)`;
  }
  animation() {
    this.setSliderPosition();
  }
